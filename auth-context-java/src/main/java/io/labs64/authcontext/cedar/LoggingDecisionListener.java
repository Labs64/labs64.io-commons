 ...
 *   <li>a <b>detail</b> line on the dedicated {@code io.labs64.authcontext.cedar.detail}
 ...
 *       so the default stream stays safe to read during the Cedar testing phase.</li>
 * </ul>
 *
...
public class LoggingDecisionListener implements AuthorizationDecisionListener {
    ...
    private static final Logger detail = LoggerFactory.getLogger("io.labs64.authcontext.cedar.detail");

    @Override
    public void onDecision(final AuthorizationDecision d) {
        String summary = String.format(
                "cedar-domain outcome=%s decision=%s mode=%s action=%s resourceType=%s reasons=%s requestId=%s",
        ...
        if (detail.isDebugEnabled()) {
            detail.debug("cedar-detail requestId={} user={} tenant={} resource={}::{}{}",
